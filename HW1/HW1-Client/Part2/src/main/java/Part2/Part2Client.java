...
public class Part2Client {
  ...
  private static final String CSV_PATH = "D:/Desktop_D/CS6650/HW/HW1-Client/Part2/src/main/java/Result/result.csv";
  ...
  public static void main(String[] args) throws InterruptedException {
    ...
    for(int i=0; i<200; i++){
      producerPool.submit(new Part2ProducerTask(producerQueue, producerRound));
    }
    producerPool.shutdown();
    /* First Round, 32 threads each sends 1k requests */
    for(int i=0; i<32; i++){
      Thread thread = new Thread(new Part2ConsumerTask(producerQueue, successCounter, failCounter, skiersApi, firstRound, 1000, recorders));
      thread.start();
    }
    firstRound.await();
    /* Second Round, After first round's done, send the rest 168k requests */
    for(int i=0; i<168; i++){
      Thread thread = new Thread(new Part2ConsumerTask(producerQueue, successCounter, failCounter, skiersApi, secondRound, 1000, recorders));
      thread.start();
    }
    secondRound.await();
    producerRound.await();
